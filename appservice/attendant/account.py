...
class AttendantAppService():
    def update(self, vm):
        try:
            ...
            audit_da.create(vm.attendant_code, 'Update Attendant', 'Fail. Error=%s' % str(ex))
            
            raise ex
        
        # audit trail, ok
        audit_da = AttendantAuditTrailDataAccess()
        audit_da.create(vm.attendant_code, 'Update Attendant', 'Ok.')
    ...
    def reset_pwd(self, vm):
        try:
            ...
            # audit trail, fail
            audit_da = AttendantAuditTrailDataAccess()
            audit_da.create(vm.attendant_code, 'Reset Attendant Password', 'Fail. Error=%s' % str(ex))
            
            raise ex
        
        # audit trail, ok
        audit_da = AttendantAuditTrailDataAccess()
        audit_da.create(vm.attendant_code, 'Reset Attendant Password', 'Ok.')
    ...
    def change_pwd(self, vm):
        try:
            ...
            # audit trail, fail
            audit_da = AttendantAuditTrailDataAccess()
            audit_da.create(vm.attendant_code, 'Change Attendant Password', 'Fail. Error=%s' % str(ex))
            
            raise ex
        
        # audit trail, ok
        audit_da = AttendantAuditTrailDataAccess()
        audit_da.create(vm.attendant_code, 'Change Attendant Password', 'Ok.')